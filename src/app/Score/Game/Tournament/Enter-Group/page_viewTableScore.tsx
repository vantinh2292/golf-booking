...
interface Props {
    ...
    luotChoi: number,
    dataConfig: IGolfDataSetupTournament,
    dataPlayers: IGolfDataPlayerTournament[],
    cellChange: (golfSetupCourse: IGolfSetupCourse, typeScore: string, index: number, data: string, name: string, hole: number, id: string) => void,
...
function Page_ViewTableScore(props: Props) {
    ...
    const [dataPlayers, setDataPlayers] = useState<IGolfDataPlayerTournament[]>([]);
    ...
    useEffect(
        () => {
            if (props.groupName.length > 0) {
                const elementsWithGroupName = Object.values(props.dataPlayers).filter(element => element.Hole === props.groupName);
                setDataPlayers(elementsWithGroupName)
                let tempTitle = ''
                if (props.luotChoi === 1) {
                    setTitle(elementsWithGroupName[0].RoundIn)
                    tempTitle = elementsWithGroupName[0].RoundIn
                    props.setTitleChange(tempTitle)
                }
                if (props.luotChoi === 2) {
                    setTitle(elementsWithGroupName[0].RoundOut)
                    tempTitle = elementsWithGroupName[0].RoundOut
                    props.setTitleChange(tempTitle)
                }
            }
        }
        , [props.dataPlayers, props.groupName]
    );

    useEffect(
        () => {
            if (title.length > 0 && props.golfName.length > 0 && dataPlayers.length > 0) {
                if (dataPlayers[0].RoundIn.length > 0) {
                    get(child(dbRef, "TSN/Score/GolfSetup/" + props.golfName + '/Course/' + dataPlayers[0].RoundIn)).then((snapshot) => {
                        if (snapshot.exists()) {
                ...
                if (dataPlayers[0].RoundOut.length > 0) {
                    get(child(dbRef, "TSN/Score/GolfSetup/" + props.golfName + '/Course/' + dataPlayers[0].RoundOut)).then((snapshot) => {
                        if (snapshot.exists()) {
                            let newData = snapshot.val();
                ...
                //Get Firebase GPS
                get(child(dbRef, "TSN/Score/GolfSetup/" + props.golfName + '/Course/' + title)).then((snapshot) => {
                    if (snapshot.exists()) {
                        let newData = snapshot.val();
        ...
        , [title, props.golfName, dataPlayers]
    );

    useEffect(
        () => {
            if (dataPlayers.length > 0 && golfSetupCourse !== null) {
                let tempArr = []
                //Tee Begin
                let isValueInArray = dataPlayers.some(obj => (obj.Gender === "M" ? props.dataConfig.TeeForMen : props.dataConfig.TeeForWomen) === golfSetupCourse.Tee1.TeeBoxName);
                ...
                isValueInArray = dataPlayers.some(obj => (obj.Gender === "M" ? props.dataConfig.TeeForMen : props.dataConfig.TeeForWomen) === golfSetupCourse.Tee2.TeeBoxName);
                ...
                isValueInArray = dataPlayers.some(obj => (obj.Gender === "M" ? props.dataConfig.TeeForMen : props.dataConfig.TeeForWomen) === golfSetupCourse.Tee3.TeeBoxName);
                ...
                isValueInArray = dataPlayers.some(obj => (obj.Gender === "M" ? props.dataConfig.TeeForMen : props.dataConfig.TeeForWomen) === golfSetupCourse.Tee4.TeeBoxName);
                ...
                isValueInArray = dataPlayers.some(obj => (obj.Gender === "M" ? props.dataConfig.TeeForMen : props.dataConfig.TeeForWomen) === golfSetupCourse.Tee5.TeeBoxName);
                ...
                isValueInArray = dataPlayers.some(obj => (obj.Gender === "M" ? props.dataConfig.TeeForMen : props.dataConfig.TeeForWomen) === golfSetupCourse.Tee6.TeeBoxName);
                ...
                dataPlayers.forEach((element, index) => {
                    tempArr.push(
                        {
                            Index: index,
                            Editable: true,
        ...
        , [golfSetupCourse, props.dataConfig, dataPlayers, props.typeScore]
    );
    ...
    const generateHtmlInputScore = (golfSetupCourse: IGolfSetupCourse | null) => {
        if (golfSetupCourse !== null) {
            return dataPlayers.map((item, index) => (
                <div key={index} style={{ display: 'flex', justifyContent: 'space-around' }}>
                    ...
                    <div style={{ overflow: 'auto', width: '100%', paddingLeft: 10 }}>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '0', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>0</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '1', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>1</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '2', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>2</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '3', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>3</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '4', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>4</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '5', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>5</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '6', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>6</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '7', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>7</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '8', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>8</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '9', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>9</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, 'CLEAR', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}>CLEAR</Button>
                        <Button style={{ margin: 3, fontWeight: 'bold' }} variant="outline-light" onClick={() => { props.cellChange(golfSetupCourse, props.typeScore, index, '-', item.NamePlayer, inputCurrentHole + (props.luotChoi - 1) * 9, item.Id) }}> - </Button>
                    </div>
                </div>
            ));
        } else {
