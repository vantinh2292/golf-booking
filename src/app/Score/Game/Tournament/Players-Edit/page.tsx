...
function Page() {
    ...
    dispatch(redux_page.setIndex(13))
    ...
    useEffect(
        () => {
            if (tournamentName.length > 0) {
                //Get Firebase GPS
                get(child(dbRef, "TSN/Score/Tournament/" + tournamentName + '/Players')).then((snapshot) => {
                    if (snapshot.exists()) {
                        let newData = snapshot.val();
    ...
    const generate_player_html = (ArrListDataPlayer: IGolfDataPlayerTournament[]) => {
        ...
        for (let index = 0; index < ArrListDataPlayer.length; index++) {
            gen_player.push(
                <div style={{ width: '100%', justifyContent: 'center', display: 'grid', margin: 1 }}>
    ...
    const EditPlayersTournament = () => {
        if (tournamentName.length == 0) {
            ...
            set(setRef, tempObj)
            toast.success('Edit Players Success');
        }
    }
    ...
    const generatePage = () => {
        if (haveData && Page_Data.level !== -1) {
            return (
                <>
                    <div style={{ color: 'white', fontSize: 25, fontWeight: 'bold', width: '100%', textAlign: 'center' }}>EDIT LIST PLAYERS</div>
                    <Spacer y={1} />
