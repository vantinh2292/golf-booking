...
function Page() {
    ...
    dispatch(redux_page.setIndex(10))
    ...
    const handleChangeHdcMin = (event: ChangeEvent<HTMLInputElement>, index: number) => {
        setHdcMin(prevState => {
            const newState = [...prevState];
            newState[index] = parseInt(event.target.value.trim());
            return newState;
        });
    }
    ...
    const AddNewGolfSetupTournament = () => {
        let check = false
        if (tournamentName.length == 0) {
            toast.warning('Tournament name can not empty');
            check = true
        }
        ...
        if (!check) {
            get(child(dbRef, "TSN/Score/Tournament")).then((snapshot) => {
                if (snapshot.hasChild(tournamentName)) {
                    toast.warning('THIS TOURNAMENT NAME ALREADY HAVE IN SYSTEM');
                } else {
                    ...
                    toast.success('Create Setup Tournament success');
                }
            }).catch((error) => {
                console.error(error);
            });
    ...
    const generatePage = () => {
        if (windowWidth > 0 && Page_Data.level !== -1) {
            return (
                <>
                    <div style={{ color: 'white', fontSize: 25, fontWeight: 'bold', width: '100%', textAlign: 'center' }}>ADD SET-UP TOURNAMENT</div>
                    <Spacer y={1} />
                        ...
                        <div className="mt-px gap-2 text-white p-2 rounded-md border-5 border-double border-slate-700"
                            ...
                            <div style={{ width: '100%', overflow: 'auto', height: windowWidth < LimitWidthWindow ? 'auto' : 'calc(100% - 40px)', maxHeight: windowWidth < LimitWidthWindow ? 'none' : heightBoxLeft - 100 }}>
                                <div className='basis-3/4'>
                                    ...
                                    <Spacer y={1} />
                                    <div style={{ justifyContent: 'center', width: '100%' }}>
                                        <div style={{ paddingTop: 5 }}>Number Prizes:</div>
                                        <Form.Select
                                            style={{
                                                width: '100%', textAlign: 'center',
                                                backgroundColor: 'rgb(59 59 59)', color: 'beige',
                    ...
                    <Spacer y={1} />
                    <Button
                        onClick={AddNewGolfSetupTournament}
                        style={{ width: '100%' }} variant="primary">ADD SET-UP TOURNAMENT
                    </Button>
                </>
            )
        } else {
